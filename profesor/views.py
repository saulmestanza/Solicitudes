...
class ProfesorProcesosListView(UserPassesTestMixin, ListView):
    model = ProcesoAlumno
    template_name = 'profesor/profesor_historial.html'
    ...
    def get_queryset(self):
        ...
        return super(ProfesorProcesosListView, self).get_queryset().all()
    
    def get_context_data(self, **kwargs):
        context = super(ProfesorProcesosListView, self).get_context_data(**kwargs)
...
class ProfesorProcesosEditView(PermissionRequiredMixin, UpdateView):
    model = ProcesoAlumno
    form_class = ProcesoAlumnoCompleteForm

    template_name = 'alumno/alumno_edit.html'
    success_url = '/profesor/profesor-seguimiento/'
    permission_required = (
    ...
    def form_valid(self, form):
        clean = form.cleaned_data
        try:
            response = super(ProfesorProcesosEditView, self).form_valid(form)
            ...
            historial = Historial.objects.create(
                ...
                name="Tramite en Estado %s"%(proceso_alumno.status), 
                description="Tramite en Estado %s"%(proceso_alumno.status),
            ...
            errors.append(u"Ya existe un usuario registrado con este email")
            response = super(ProfesorProcesosEditView, self).form_invalid(form)
        return response

    def form_invalid(self, form):
        return super(ProfesorProcesosEditView, self).form_invalid(form)

    def get_context_data(self, **kwargs):
        context = super(ProfesorProcesosEditView, self).get_context_data(**kwargs)
